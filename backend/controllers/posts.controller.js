...
// GET /api/posts?forumType=&category=&search=&sortBy=
// Returns approved posts for ONE forum, with optional category + search.
async function getPosts(req, res) {
	const { category, search, userId, forumType, sortBy } = req.query;
	if (forumType && !FORUM_TYPES.includes(forumType)) {
		return res.status(400).json({ error: "forumType must be 'study' or 'habit'." });
	}
	const result = await postsRepo.findApproved({ category, search, userId, forumType, sortBy });
	res.json(result);
}
...
async function getPost(req, res) {
	...
	const post = await postsRepo.findById(Number(req.params.id), userId);
