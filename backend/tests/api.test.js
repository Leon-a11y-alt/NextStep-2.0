...
test("a user can like a comment once and then remove that like", async () => {
  ...
  const first = mockRes();
  await commentsCtrl.likeComment({ params: { id: String(createRes.body.id) }, body: { userId: 9 } }, first);
  assert.strictEqual(first.statusCode, 200);
  assert.strictEqual(first.body.likes, 1);

  const second = mockRes();
  await commentsCtrl.likeComment({ params: { id: String(createRes.body.id) }, body: { userId: 9 } }, second);
  assert.strictEqual(second.statusCode, 200);
